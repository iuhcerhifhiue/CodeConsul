  ...
  return (
    <div>
      ...
      {expanded && !hideDetails && (
        <div className="ml-4 mt-0.5 mb-1 border-l-2 border-gray-100 pl-3 space-y-1.5">
          {Object.keys(args).length > 0 && (
            <div>
              <p className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">Input</p>
              <pre className="text-[11px] text-gray-600 bg-gray-50 rounded p-2 overflow-x-auto border border-gray-100 max-h-40 overflow-y-auto">{JSON.stringify(args, null, 2)}</pre>
            </div>
          )}
          {results !== undefined && results !== null && (
            <div>
              <p className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">Output</p>
              <pre className="text-[11px] text-gray-600 bg-gray-50 rounded p-2 overflow-x-auto border border-gray-100 max-h-40 overflow-y-auto">{typeof results === 'string' ? results : JSON.stringify(results, null, 2)}</pre>
            </div>
          )}
        </div>
      )}
